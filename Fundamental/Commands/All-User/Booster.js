...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    let rol = data.BOOST;
    if (!message.member.roles.cache.has(rol) && !message.member.permissions.has("ADMINISTRATOR")) return message.reply("Bu komutu kullanabilmek için gerekli izne sahip değilsin!");
    var isim = args.slice(0).join(" ");
    if(!isim) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Geçerli bir isim belirt ve tekrar dene.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
