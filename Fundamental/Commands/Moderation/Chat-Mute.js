...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sec == "setup") {
        if (!args[1]) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Geçerli bir kurulum belirt.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has("ADMINISTRATOR") || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.MUTEAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ravgar_carpi"));
                    return data.MUTEChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ravgar_carpi"));
                    return data.MUTELimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
            })
        } else return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Bu komutu yanlızca bot sahibi kullanabilir.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
        } else {
            let server = await sunucuayar.findOne({guildID: message.guild.id});  
            if(!message.member.permissions.has("8") && !server.MUTEAuthorized.some(rol => message.member.roles.cache.has(rol))  &&!server.UstYetkiliRol.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "ravgar_cancel")}`)    
        ...
        if (!time) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Geçerli bir süre belirt ve tekrar dene.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
        ...
        let muteSorumlusu = data.MUTEAuthorized;
                    ...
                    if (muteSorumlusu.length >= 1 && client.channels.cache.get(muteLogKanal) && muteLimit >= 1) {
                        if (target.roles.cache.get(muteROL)) return message.react(client.emojis.cache.find(res => res.name === "ravgar_carpi"))
                        if (limit.get(`${message.author.id}`) >= muteLimit) return message.reply(`${client.emojis.cache.find(res => res.name === "ravgar_carpi")} Geçerli olan \`Mute limitini\` aştığın için işlem iptal edildi.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
                        ...
                        let messageEmbed = new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({
                            dynamic: true
                        })).setDescription(`${client.emojis.cache.find(res => res.name === "ravgar_mute")} ${target} (**${target.id}**) kullanıcısı ${message.author} tarafından **${reason}** sebebiyle geçiçi olarak susturuldu. (Ceza Numarası: \`#${cezaID+1}\`)`)
