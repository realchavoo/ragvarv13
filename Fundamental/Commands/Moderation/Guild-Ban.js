...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sec == "setup") {
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.REKLAMAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ravgar_carpi"));
                    return data.REKLAMChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ravgar_carpi"));
                    return data.REKLAMLimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
            })
      ...
      if(!message.member.permissions.has("8") && !server.REKLAMAuthorized.some(rol => message.member.roles.cache.has(rol))  &&!server.UstYetkiliRol.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "ravgar_cancel")}`)
    if (jailSorumlusu.length >= 1 && client.channels.cache.get(reklamLOGKanal) && REKLAMLimit >= 1) {
            ...
            if (!reason) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Geçerli bir sebeb belirt ve tekrar dene.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
            if (limit.get(message.author.id) >= REKLAMLimit) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Limiti aştıgın için ban komutunu kullanman engellendi.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
            ...
            if (target.roles.cache.get(data.EnAltYetkiliRol) && !message.member.permissions.has(8)) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Yetkili birine işlen uygulayamazsın.`);
            if (target.roles.cache.get(reklamROL)) return message.react(client.emojis.cache.find(res => res.name === "ravgar_carpi"));

            let embedcik = new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({
                dynamic: true
            })).setDescription(`${client.emojis.cache.find(res => res.name === "ravgar_ban")} ${target} (**${target.id}**) kullanıcısı ${message.author} tarafından **${reason}** sebebiyle sunucudan yasaklandı. (Ceza Numarası: \`#${cezaID+1}\`)`)
...
let messageLogEmbed = new MessageEmbed()
				...
				.setTimestamp()
				.setDescription(`${target} (**${target.id}**) üyesi __**${moment(Date.now()).format('LLL')}**__ tarihinde ${message.author} (**${message.author.id}**) tarafından **${reason}** sebebiyle geçiçi olarak cezalı kategorisine gönderildi.`)
