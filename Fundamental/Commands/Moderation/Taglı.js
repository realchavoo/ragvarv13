...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if(!message.member.permissions.has("8") && !message.member.roles.cache.has(server.EnAltYetkiliRol) && !server.BANAuthorized.some(rol => message.member.roles.cache.has(rol))  &&!server.UstYetkiliRol.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "ravgar_cancel")}`)
        ...
        if (target.id === message.author.id) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Kendini taglı olarak kayıt edemezsin.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
		if (!target.user.username.includes(data.TAG)) return  message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Belirttiğin üye sunucu tagımızı kullanıcı adına eklememiş. (\`${data.TAG}\`)`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
        ...
        if(button.customId === "onaylatagli") {
            client.channels.cache.find(a => a.name === "tag-log").send(`${client.emojis.cache.find(x => x.name === "ravgar_tik") || "Emoji Bulunamadı"} ${message.author} adlı yetkili ${target} adlı üyeyi taglı olarak kaydetti.`)
...
await baddAudit(message.author.id, 1)
            msg.delete()
            button.channel.send({embeds: [embed3.setDescription(`${client.emojis.cache.find(x => x.name === "ravgar_tik") || "Emoji Bulunamadı"} ${target} adlı üye onu taglı olarak kaydetmeni onayladı.`)] }), message.react(`${client.emojis.cache.find(x => x.name === "ravgar_tik")}`)
        ...
        msg.delete()
        button.channel.send({embeds: [embed2.setDescription(`${client.emojis.cache.find(x => x.name === "ravgar_carpi") || "Emoji Bulunamadı"} ${target} adlı üye onu taglı olarak kaydetmeni reddetti.`)] }), message.react(`${client.emojis.cache.find(x => x.name === "ravgar_tik")}`)}})}}
exports.conf = {
    aliases: ["Taglı"]
}
exports.help = {
