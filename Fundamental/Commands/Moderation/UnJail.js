...
module.exports.run = async (client, message, args, durum, kanal) => {
	...
	if (await client.permAyar(message.author.id, message.guild.id, "jail") || durum) {
        ...
        if (!target) return message.reply(`${client.emojis.cache.find(x => x.name === "ravgar_cancel")} Geçerli bir **Üye Belirt** ve tekrar dene.`);
		if (!target.roles.cache.get(jailRol)) return message.reply(`${client.emojis.cache.find(x => x.name === "ravgar_cancel")} Belirttiğin üye cezalıda değil.`)
