...
if (target.roles.cache.get(data.EnAltYetkiliRol) && !message.member.permissions.has("ADMINISTRATOR")) return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Yetkililer birbilerini kayıtsıza atamaz.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
    otokayit.deleteOne({userID: target.id}).exec();
        target.setNickname(`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} İsim | Yaş`)
    target.roles.set(unRegisterRol).then(x => message.react(client.emojis.cache.find(x => x.name === "ravgar_tik"))).catch(y => message.react(client.emojis.cache.find(x => x.name === "ravgar_iptal")));
}
